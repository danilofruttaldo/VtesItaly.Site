...
2. **`lighthouse`** — depends on `ci`. Downloads the `dist/` artifact and runs Lighthouse CI from `.github/lighthouserc.json` over 8 URLs (home IT/EN, principati, cartoline, comunita, current tour and nazionale editions, plus a long article). Accessibility ≥ 0.89, SEO ≥ 0.9, performance ≥ 0.75 are blocking errors; best-practices is a warning. The 0.75 performance gate landed after the November 2026 PostCard thumbnail + hero mobile-variant rollout dropped homepage LCP from ~3.6 s to ~3.0 s and CLS from 0.07 to 0. Bumping further would require critical-CSS inlining (Base.css + Calendar.css still block ~810 ms of render) or deferring the Calendar bundle on the homepage. `throttlingMethod: devtools` is pinned in the LHCI config: the default `simulate` mode multiplies its CPU-slowdown factor by a `benchmarkIndex` measured at runtime, so the first URL on a cold Ubuntu runner gets a 20 %-slower benchmark than later URLs and its TBT explodes — the same `/` page that scored 0.73 on a cold run scores 0.98 as `/en/` once the runner warms up. `devtools` applies a fixed multiplier at the Chrome level, removing the variance source.
