---
title: 'Grand Prix 2025'
date: 2025-03-01
category: 'grand-prix'
tags: ['italia', 'modena']
excerpt: 'The Italian Grand Prix 2025 was held in Modena on Saturday 1 March 2025. Federico wins! Congratulations to the finalists.'
featuredImage: '/gp/gp-2025/gp-2025-featured.webp'
poster: '/gp/gp-2025/gp-2025-poster.webp'
venue:
  name: 'Hotel Baia del Re ****'
  address: 'Str. Vignolese, 1684, 41126 Modena (MO)'
  siteUrl: 'https://www.hotelbaiadelre.com/'
  mapUrl: 'https://maps.google.com/?q=Hotel+Baia+del+Re+Str.+Vignolese+1684+Modena'
events:
  - name: 'MAIN EVENT: Italian Grand Prix 2025'
    date: 2025-03-01
    time: '09:30'
    format: 'Standard'
    proxies: false
    rounds: 3
    archonUrl: 'https://gp2025.vtesitaly.com/'
  - name: 'SIDE EVENT: Italian Tour 2025 - Modena'
    date: 2025-03-02
    time: '09:00'
    format: 'Standard'
    proxies: true
    rounds: 2
    archonUrl: 'https://archon.vekn.net/tournament/5e7c9f73-b30e-4657-90a9-f60545309b82/display.html'
pricing:
  - label: 'Main Event + lunch'
    regular: '60.00 euro'
  - label: 'Side Event + lunch'
    regular: '35.00 euro'
payment: 'PayPal (friends and family) — vtesitaly@gmail.com'
hotel:
  - room: 'Hotel Baia del Re **** (event venue)'
    price: 'discounted with code VTES25'
  - room: 'Villa Aurora B&B (5 min walk)'
    price: ''
  - room: 'B&B Anna (5 min by car)'
    price: ''
hotelNote: 'Discounted reservations: compile the module using code VTES25.'
prizes:
  - 'Welcome pack: GP circuit playmat, promo card, custom pens'
  - "Original 'One With the Land' art by Riccardo Fabiani"
  - 'Commemorative metal rings for top 10'
  - 'Sponsored UltraPro material'
  - 'Timer for 3rd round winners'
registration: 'Pre-registration and decklist required'
contact: 'info@vtesitaly.com'
standingsUrl: '/gp/gp-2025/standings-gp-2025.json'
notes:
  - "200m from 'Modena Sud' highway exit. Bologna Marconi airport 35 km away. Extra-urban bus #731 to Vignola, 'Ponte Guerro' stop 500m from venue."
links:
  - label: 'Modulo Prenotazione Hotel'
    url: '/gp/gp-2025/modulo-prenotazione-gp-2025.doc'
    locale: it
  - label: 'Hotel Reservation Form'
    url: '/gp/gp-2025/reservation-form-gp-2025.doc'
    locale: en
gallerySections:
  - title: 'Setup'
    images:
      - '/gp/gp-2025/gp-2025-preparazione-01.webp'
      - '/gp/gp-2025/gp-2025-preparazione-02.webp'
      - '/gp/gp-2025/gp-2025-preparazione-03.webp'
    wideImages:
      - '/gp/gp-2025/gp-2025-preparazione-03.webp'
  - title: 'Rounds'
    images:
      - '/gp/gp-2025/gp-2025-round-01.webp'
      - '/gp/gp-2025/gp-2025-round-02.webp'
      - '/gp/gp-2025/gp-2025-round-03.webp'
      - '/gp/gp-2025/gp-2025-round-04.webp'
      - '/gp/gp-2025/gp-2025-round-05.webp'
      - '/gp/gp-2025/gp-2025-round-06.webp'
  - title: 'Final'
    images:
      - '/gp/gp-2025/gp-2025-finale-01.webp'
      - '/gp/gp-2025/gp-2025-finale-02.webp'
      - '/gp/gp-2025/gp-2025-finale-03.webp'
      - '/gp/gp-2025/gp-2025-finale-04.webp'
      - '/gp/gp-2025/gp-2025-finale-05.webp'
      - '/gp/gp-2025/gp-2025-finale-06.webp'
  - title: 'Bonus'
    images:
      - '/gp/gp-2025/gp-2025-bonus-01.webp'
      - '/gp/gp-2025/gp-2025-bonus-02.webp'
      - '/gp/gp-2025/gp-2025-bonus-03.webp'
      - '/gp/gp-2025/gp-2025-bonus-04.webp'
      - '/gp/gp-2025/gp-2025-bonus-05.webp'
      - '/gp/gp-2025/gp-2025-bonus-06.webp'
locale: en
---

The **Italian V:TES Community**, in collaboration with **Ultra-Pro**, **Kraken Nest**, **Card Game Geek** and **Mistlore**, is proud to present the **GP 2025** in **Modena** at **Hotel Baia del Re**. A day of challenges, alliances and betrayals among the fiercest Methuselahs of the Old World.
